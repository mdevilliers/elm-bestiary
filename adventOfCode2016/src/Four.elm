...
partone =
    load
        ...
        |> List.map
            (\l ->
                case String.contains l.bits l.checksum of
                    True ->
                        l.sectorID
...
stringToSectorID str =
    Result.withDefault 0 (String.toInt (String.filter (\c -> Char.isDigit c) str))
...
parseBits str =
    String.filter (\c -> c /= '-') str
        ...
        |> List.reverse
        |> List.take 5
        |> List.foldl (\( v, _ ) acc -> acc ++ (String.fromChar v)) ""


comparer ( ak, av ) ( bk, bv ) =
...
groupBy : String -> Dict Char Int
groupBy str =
    String.foldl
        (\s dict ->
            if Dict.member s dict then
                Dict.update s (Maybe.map ((+) 1)) dict
            else
                Dict.insert s 1 dict
        )
        Dict.empty
        str
